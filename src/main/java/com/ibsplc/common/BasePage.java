...
public abstract class BasePage {
	...
	protected void click(WebElement element) {
		try {
			...
			InitialSetup.test.log(LogStatus.FAIL, "Unable to click" + e);
	...
	protected void enterKeys(By by, String keys) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("An interupted Exception occured" + e);
			InitialSetup.test.log(LogStatus.FAIL, "Unable to send the data" + e);
	...
	protected String getAttributebyValue(By by) {
		try {
			...
			System.out.println("An exception Occured" + e);
			InitialSetup.test.log(LogStatus.FAIL, "An exception Occured" + e);
			return null;
		}
	}
	...
	public void captureAndAddScreenshot() {
		try {
			...
			BaseSetup.test.log(LogStatus.INFO, BaseSetup.test.addScreenCapture(dir + path));
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println(e);
		}
...
//			            int cellIndex = row1.getLastCellNum();
				 ...
				 * @param newValue
				 */
		public void modifyXmlFile(String filePath,String tagName,String newValue){
		        ...
		        updateElementValue(doc,tagName,newValue);
		        } catch (Exception e1) {
		            e1.printStackTrace();
		        }
		}
		 ...
		 * @param newValue
		 */
		 private void updateElementValue(Document doc,String tagName,String newValue) {
		        NodeList users = doc.getElementsByTagName("User");
		        Element user = null;
		        // loop for each user
		        for (int i = 0; i < users.getLength(); i++) {
		            user = (Element) users.item(i);
		            Node name = user.getElementsByTagName(tagName).item(0).getFirstChild();
