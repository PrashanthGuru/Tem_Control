...
public class OPR026 extends BasePage {
	...
	public OPR026(WebDriver driver, String dataFileName) {
		super(driver);
		this.driver = driver;
		initElements();
	 ...
	 * A-8255
	 */
	public OPR029 capture_HAWB_details(String stockType, String awbPre, String awbNo, String origin, String dest,
			String carrCode, String agentCode, String shipper, String consignee, String pcs, String wt, String commCode,
			boolean isConsol, boolean with_HAWB) {
		...
		if (verifyElementPresent(btn_CloseCaptureHouseNavigate)) {
			click(btn_CloseCaptureHouseNavigate);
	...
	public OPR292 relistAWBandNavigateToOPR292(String awbPre, String awbNo) {
	...
	public OPR026 ProcessasIsExecute(String prefix, String awbno, String shipper, String consignee, String sci,
		...
		InitialSetup.test.log(LogStatus.PASS, "Status is Executed successfully" + actaulValue);
		return this;
	}
	...
	public OPR026 ProcessasIsExecuteafterBooking(String prefix, String awbno, String shipper, String consignee) {
		...
		maxWait();
		maxWait();
		InitialSetup.test.log(LogStatus.INFO, "Successfully entered Prefix and AWB no in OPR026 screen");
		// if (verifyElementVisible(txt_shipper))
		if (waitForElement(txt_shipper).isDisplayed()) {
			enterKeys(txt_shipper, shipper + Keys.TAB);
			enterKeys(txt_consignee, consignee + Keys.TAB);
		...
		String actaulValue = driver.findElement(By.xpath("(//label[text()='Status']//following-sibling::span/span)[2]"))
				.getText();
		Assert.assertTrue(isFound, "Executed" + actaulValue);
		InitialSetup.test.log(LogStatus.PASS, "AWB is Executed successfully: " + actaulValue);
		return this;
	}
	...
	public OPR026 ProcessExecuteafterBooking(String prefix, String awbno, String shipper, String consignee) {
		...
		maxWait();
		maxWait();
		InitialSetup.test.log(LogStatus.INFO, "Starting with the capture AWB process");
		if (waitForElement(txt_shipper).isDisplayed()) {
			enterKeys(txt_shipper, shipper + Keys.TAB);
			enterKeys(txt_consignee, consignee + Keys.TAB);
			minWait();
		...
		InitialSetup.test.log(LogStatus.INFO, "Entered the shipper and consignee details");
//		click(btn_chargeTab);
//		minWait();
//		click(chcbx_Autorate);
//		click(btn_btnAutoRate);
		...
		click(btn_execute);
		InitialSetup.test.log(LogStatus.INFO, "Clicked on button : execute");
		continueEmbargo();
			handleAlert("Accept", "OPR026");
			waitForFrameAndSwitch(screenFrame);
		maxWait();
		...
		try {
			...
			if (verifyElementPresent(
				...
				InitialSetup.test.log(LogStatus.WARNING, getText_JS(
						By.xpath("//span[contains(text(),'Suspicious')]/../..//td[contains(@class,'error')]")));
		...
		if (actualValue.equalsIgnoreCase("Executed")) {
			InitialSetup.test.log(LogStatus.PASS, "AWB is Executed successfully with status : " + actualValue);
			captureAndAddScreenshot();
		} else {
			InitialSetup.test.log(LogStatus.PASS, "AWB isn't Executed successfully and has status : " + actualValue);
	...
	public OPR026 asIsExecuteInOPR026(String prefix, String awbno, String stockType, String shipper, String consignee,
		...
		// Executed.");
		InitialSetup.test.log(LogStatus.PASS, "AWB No :" + awbno + " is executed successfully");
		captureAndAddScreenshot();
		minWait();
		return this;
	}
	...
	public void captureChecksheetForAvi() {
		...
		selectByText(
				By.xpath(
						"//input[contains(@value,'Was a paper Shipper Security Endorsement (SSE) completed')]/parent::label/../following-sibling::div[1]/select"),
				"Yes");
		selectByText(
				By.xpath(
						"//input[contains(@value,'Were forecasted temps')]/parent::label/../following-sibling::div[1]/select"),
				"Yes");
		/*
		 * click(By.xpath("(//span[@class='ui-icon ui-icon-triangle-1-s'])[1]"))
		 * ; check(By.xpath(
		 * "//input[@name='multiselect_CMP_Checksheet_Defaults_CaptureCheckSheet_MultiSelect2'][@value='25Ñ†/-3.9Ñƒ']"
		 * ));
	...
	public OPR026 FinalizePayment() {
		...
		if (status.equalsIgnoreCase("Final")) {
			InitialSetup.test.log(LogStatus.PASS, "AWB is Finalized with Cash : " + status);
			captureAndAddScreenshot();
		} else {
			InitialSetup.test.log(LogStatus.FAIL, "AWB isn't Executed successfully and has status : " + status);
	...
	public OPR026 ProcessExecuteAfterCreditCardPayment(String prefix, String awbno, String shipper, String consignee) {
		 ...
		 * click(btn_execute); InitialSetup.test.log(LogStatus.INFO,
	 ...
	 * @return
	 */
	public CSH007 PaymentByCCard(String prefix, String awbNo) {
		InitialSetup.test.log(LogStatus.INFO, "Successfully listed at CSH007 Screen");
		maxWait();
		click(By.xpath("//*[@id='paymentSection']//a//div//h2[contains(text(),'CCard')]"));
		...
		if ((paymentAdviceNo != "")) {
			PropertyHandler.setPropValue(dataFilePath, "PaymentAdviceNo", paymentAdviceNo);
			InitialSetup.test.log(LogStatus.INFO, "Payment made by CASH and Payment Advice Number is: ");
		} else {
			InitialSetup.test.log(LogStatus.FAIL, "Could not do payment");
			Assert.fail();
		}
...
//					waitForFrameAndSwitch(screenFrame);
					...
					if(alertText.contains("AWB 001-"+awbno+" is executed. Do you want to relist?")){
						InitialSetup.test.log(LogStatus.PASS, "AWB is Executed successfully with status : " + alertText);
						captureAndAddScreenshot();
					}
					else{
						InitialSetup.test.log(LogStatus.PASS, "AWB isn't Executed successfully and has status : " + alertText);
...
//		waitForFrameAndSwitch(screenFrame);
		...
		if(verifyElementPresent(By.xpath("//*[contains(text(),'Dangerous goods details not captured for the SCC')]"))){
			InitialSetup.test.log(LogStatus.INFO, "handling DG popup");
			captureAndAddScreenshot();
			click(By.xpath("//button[text()='Yes']"));
			minWait();
		}
