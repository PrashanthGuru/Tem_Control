...
//    	check(chkbox_final);
    	...
    	InitialSetup.test.log(LogStatus.PASS, "Successfully saved the details");
    	...
    	if(flag.equalsIgnoreCase("moreWt"))
			if (verifyElementPresent(By.xpath("//td[@class='ic-error-msg']"))) {
				...
				InitialSetup.test.log(LogStatus.PASS, "Validtion passed with message: " + status);
				captureAndAddScreenshot();
			}else{
				InitialSetup.test.log(LogStatus.FAIL, "No error message popped up for weight greater than threshold");
			}
    	else{
			click(btn_generateNOTOC);
			InitialSetup.test.log(LogStatus.PASS, "Successfully clicked on Generate Notoc button");
			captureAndAddScreenshot();
			driver.switchTo().defaultContent();
			if (verifyElementPresent(yesBtn))
				click(yesBtn);
			...
			waitForNewWindow(2);
			switchToSecondWindow();
			InitialSetup.test.log(LogStatus.PASS, "Generate notoc was successful");
			captureAndAddScreenshot();
			closeAndmoveToFirstWindow();
			waitForFrameAndSwitch(FRAME);
    	}
